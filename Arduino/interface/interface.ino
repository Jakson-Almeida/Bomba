// ESP32-S3 + L298N — 6 bombas peristálticas
// PWM 12 bits no pino ENA; IN1/IN2 definem o sentido.
// Serial USB 115200, uma linha por comando.
...
// Ajuste os pinos conforme a fiação. Motor 1 e 2 seguem o sketch 2_motores.
// ENA recebe PWM. IN1/IN2 são digitais (direção).
struct MotorPins {
  int ena;
  int in1;
  int in2;
...
void setup() {
  ...
  for (int i = 0; i < MOTOR_COUNT; i++) {
    ...
    ledcAttach(PINS[i].ena, PWM_FREQ, PWM_RES);
    ledcWrite(PINS[i].ena, 0);
    motors[i].enabled = false;
